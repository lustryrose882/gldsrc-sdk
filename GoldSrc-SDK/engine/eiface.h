...
typedef struct enginefuncs_s
{
	...
	void		(*pfnDeltaAddEncoder)		(const char *name, void (*conditionalencode)(struct delta_s *, const unsigned char *from, const unsigned char *to));
...
//	const char *(*pfnKeyNameForBinding)					(const char* pBinding);
	...
	int			(*pfnEngCheckParm)						(const char *pchCmdLineToken, const char **ppnext);

	// Added 2019/07/24 (no SDK update):
...
typedef struct
{
	...
	int				(*pfnConnectionlessPacket)	(const struct netadr0_s *net_from_, const char *args, char *response_buffer, int *response_buffer_size);
