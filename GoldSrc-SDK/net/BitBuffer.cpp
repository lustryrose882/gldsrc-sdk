...
bool BitBuffer::Resize(unsigned int size)
{
	...
	m_Data = (unsigned char *)malloc(size + 4);
...
void BitBuffer::WriteBits(unsigned int data, int numbits)
{
	...
	if (m_LittleEndian)
	{
		...
		if (bits <= 32)
		{
			*(uint32_t *)m_CurByte |= (ROWBITTABLE[numbits] & data) << m_CurBit;
		...
		else
		{
			*(uint32_t *)m_CurByte |= (ROWBITTABLE[numbits] & data) << m_CurBit;
			...
			m_CurByte += 4;
			*(uint32_t *)m_CurByte |= (ROWBITTABLE[numbits] & data) >> leftBits;
		}
...
{
	WriteBits(c, 8 * sizeof(uint8_t));
}

void BitBuffer::WriteShort(int c)
{
	WriteBits(c, 8 * sizeof(int16_t));
}

void BitBuffer::WriteWord(int c)
{
	WriteBits(c, 8 * sizeof(uint16_t));
}

void BitBuffer::WriteLong(unsigned int c)
{
	WriteBits(c, 8 * sizeof(uint32_t));
}
...
void BitBuffer::WriteFloat(float f)
{
	...
	dat.f = f;
	WriteBits(LittleFloat(dat.i), 8 * sizeof(float));
}
...
{
	WriteByte((int64_t)(fmod((double)f, 360.0) * 256.0 / 360.0) & 0xFF);
}

void BitBuffer::WriteHiresAngle(float f)
{
	WriteShort((int64_t)(fmod((double)f, 360.0) * 65536.0 / 360.0) & 0xFFFF);
}
