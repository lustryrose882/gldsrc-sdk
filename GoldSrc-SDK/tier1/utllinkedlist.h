...
#include "tier1/utliterator.h"
	...
	#define LL_INDEX_TYPE(listName) int
#else
	#define LL_INDEX_TYPE(listName)\
		decltype((listName).MaxElementIndex())
#endif
...
#define FOR_EACH_LL(listName, iteratorName) \
	for (LL_INDEX_TYPE(listName) iteratorName = (listName).CUtlLinkedListHead(); iteratorName != (listName).InvalidIndex(); iteratorName = (listName).Next(iteratorName))

#define FOR_EACH_LL_BACK(listName, iteratorName) \
	for (LL_INDEX_TYPE(listName) iteratorName = (listName).CUtlLinkedListTail(); iteratorName != (listName).InvalidIndex(); iteratorName = (listName).Previous(iteratorName))

#define FOR_EACH_LL_FAST(listName, iteratorName) \
	for (LL_INDEX_TYPE(listName) iteratorName = 0; iteratorName < (listName).MaxElementIndex(); iteratorName++) if (!(listName).IsValidIndex(iteratorName)) continue; else
...
template <class T, class I>
void CUtlLinkedList<T, I>::PurgeAndDeleteArray()
{
	int iNext;
	for (int i = Head(); i != InvalidIndex(); i = iNext)
	{
...
// Swap one list for another
template <class T, class I>
void CUtlLinkedList<T, I>::Swap(CUtlLinkedList<T, I> &list)
{
...
template <class T, class I>
T CUtlLinkedList<T, I>::RemoveFromHead()
{
...
template <class T, class I>
T CUtlLinkedList<T, I>::RemoveFromTail()
{
