...
constexpr auto CRC32_INIT_VALUE = CRC32_t{0xFFFFFFFFUL};
constexpr auto CRC32_XOR_VALUE  = CRC32_t{0xFFFFFFFFUL};
...
bool CRC_File(CRC32_t *crcvalue, const char *pchFileName)
{
	...
	while (ulFileSize > 0)
	{
		if (ulFileSize > sizeof(chunk))
			nBytesRead = fread(chunk, 1, sizeof(chunk), fp);
		else
			nBytesRead = fread(chunk, 1, ulFileSize, fp);

		if (nBytesRead > 0)
		{
			ulFileSize -= nBytesRead;
