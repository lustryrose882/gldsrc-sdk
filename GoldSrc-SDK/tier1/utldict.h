...
#define FOR_EACH_DICT(dictName, iteratorName) \
	for (MAP_INDEX_TYPE(dictName) iteratorName = dictName.First(); iteratorName != dictName.InvalidIndex(); iteratorName = dictName.Next(iteratorName))

// faster iteration, but in an unspecified order
#define FOR_EACH_DICT_FAST(dictName, iteratorName) \
	for (MAP_INDEX_TYPE(dictName) iteratorName = 0; iteratorName < dictName.MaxElement(); iteratorName++) if (!dictName.IsValidIndex(iteratorName)) continue; else
...
template <class T, class I>
void CUtlDict<T, I>::PurgeAndDeleteArray()
{
	// Delete all the elements.
	I index = m_Elements.FirstInorder();
	while (index != m_Elements.InvalidIndex())
