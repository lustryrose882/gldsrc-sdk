...
{
	unsigned long i = 0;
	unsigned char finalcount[] = { 0, 0, 0, 0, 0, 0, 0, 0 };
...
const char *CSHA1::RawStringHash()
{
	static char hash[k_cubHash * 2] = {0};
	...
	{
		sprintf(szTemp, "%02x", m_digest[i]);
		strncat(hash, szTemp, sizeof(hash) - strlen(hash));
	}
