...
template <typename T>
T Sys_GetProcAddress(void *pModuleHandle, const char *pName)
{
	return reinterpret_cast<T>(GetProcAddress((HMODULE)pModuleHandle, pName));
}

template <typename T>
T Sys_GetProcAddress(CSysModule *pModuleHandle, const char *pName)
{
	return reinterpret_cast<T>(GetProcAddress((HMODULE)pModuleHandle, pName));
}
...
extern "C"
{
	EXPORT_FUNCTION IBaseInterface *CreateInterface(const char *pName, int *pReturnCode = nullptr);
};

PLATFORM_INTERFACE CreateInterfaceFn GetCrashHandlerFactory();
...
extern const char *Sys_GetLastErrorMsg();
extern bool Sys_UnloadModule(CSysModule *pModule);
extern CreateInterfaceFn Sys_GetFactory(CSysModule *pModule);

// Finds a particular interface in the factory set
template <typename T = void>
inline T *InitializeInterface(const char *interfaceName, CreateInterfaceFn *factoryList, int numFactories, CreateInterfaceFn *pGetterFactoryFn = nullptr)
{
	T *retval = nullptr;
	if (pGetterFactoryFn) *pGetterFactoryFn = nullptr;

	for (int i = 0; i < numFactories; i++)
	{
		auto factory = factoryList[i];
		if (!factory)
			continue;

		retval = static_cast<T *>(factory(interfaceName, nullptr));
		if (retval)
		{
			if (pGetterFactoryFn) *pGetterFactoryFn = factory;
			return retval;
		}
	}
...
{
public:
	virtual ~IFactoryList() {}
	virtual int Count() const = 0;
	virtual CreateInterfaceFn *Get() const = 0;
	virtual IBaseInterface *Find(const char *pName, int *pReturnCode = nullptr) const = 0;

	operator CreateInterfaceFn *() const
	{
		return Get();
	}
};

class CFactoryList: public IFactoryList
{
public:
	virtual int Count() const { return m_Count; }
	virtual CreateInterfaceFn *Get() const { return m_FactoryList; }

	void Add(CreateInterfaceFn *factories, int numFactories = 1);

	template <typename T>
	T *Initialize(const char *pName, int *pReturnCode = nullptr)
	{
		return reinterpret_cast<T *>(Find(pName, pReturnCode));
	}

protected:
	virtual IBaseInterface *Find(const char *pName, int *pReturnCode) const;
	bool FindDuplicate(CreateInterfaceFn factory) const;

private:
	enum { MAX_FACTORIES = 20 };

	int m_Count;
	mutable CreateInterfaceFn m_FactoryList[MAX_FACTORIES];
};

extern CFactoryList g_FactoryList;
