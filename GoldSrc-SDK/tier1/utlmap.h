...
public:
	...
	// CUtlString helper
	ElemType_t			operator[](CUtlString key);
	const ElemType_t &	operator[](CUtlString key) const;
...
inline T CUtlMap<K, T, I, L>::operator[](CUtlString key)
{
...
template <typename K, typename T, typename I, typename L>
inline const T &CUtlMap<K, T, I, L>::operator[](CUtlString key) const
{
	IndexType_t index = Find(key);
	if (index == InvalidIndex())
	{
...
// Same as CUtlMap, but less func defaults to be CDefLess instead of function pointer
