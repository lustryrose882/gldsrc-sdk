...
#if !defined DID_VEC4_T_DEFINE && !defined vec4_t
#define DID_VEC4_T_DEFINE
typedef vec_t vec4_t[4];
#endif

#if !defined DID_FIXED16_T_DEFINE && !defined fixed16_t
#define DID_FIXED16_T_DEFINE
...
FORCEINLINE void VectorInverse(vec_t *in)
{
	in[0] = -in[0];
	in[1] = -in[1];
	in[2] = -in[2];
...
FORCEINLINE void VectorAverage(const vec_t *veca, const vec_t *vecb, vec_t *out)
{
...
FORCEINLINE bool VectorIsZero(const vec_t *in)
{
...
{
	return sqrtf(v[0] * v[0] + v[1] * v[1] + v[2] * v[2] + FLT_EPSILON);
}
...
// NOTE: the old code took an int, and if you pass in an int of 0x80000000 casted to a uint32_t,
...
template <>
FORCEINLINE QAngle Lerp<QAngle>(float flPercent, const QAngle &q1, const QAngle &q2)
{
...
template <>
FORCEINLINE QAngleByValue Lerp<QAngleByValue>(float flPercent, const QAngleByValue &q1, const QAngleByValue &q2)
{
	// Avoid precision errors
	if (q1 == q2)
		return q1;
...
{
	return
		vOrigin.x - flRadius < bbMax.x && vOrigin.x + flRadius > bbMin.x &&
...
// Init the mathlib
void MathLib_Init(bool bAllow3DNow = true, bool bAllowSSE = true, bool bAllowSSE2 = true, bool bAllowMMX = true);
