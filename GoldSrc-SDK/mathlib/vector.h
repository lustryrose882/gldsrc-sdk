...
#define Z_INDEX	2
	...
	#define CHECK_VALID(_v)	DbgAssert((_v).IsValid())
#else
	#ifdef GNUC
...
public:
	...
	FORCEINLINE Vector &operator=(const float *rgfl);
...
#if !defined(VECTOR_NO_SLOW_OPERATIONS)
	// copy constructors
...
private:
	...
	Vector(const Vector &vOther);
#endif // #if !defined(VECTOR_NO_SLOW_OPERATIONS)
};
...
inline void Vector::CopyFromArray(float *rgfl)
{
	DbgAssert(rgfl);
	CHECK_VALID(*this);
...
FORCEINLINE Vector &Vector::operator=(const float *rgfl)
{
	CHECK_VALID(*this);
...
{
	CHECK_VALID(v);
	DbgAssert(v.x != 0.0f && v.y != 0.0f && v.z != 0.0f);
	x /= v.x;
	y /= v.y;
	z /= v.z;
	CHECK_VALID(*this);
...
FORCEINLINE ShortVector &ShortVector::operator/=(const ShortVector &v)
{
	DbgAssert(v.x != 0 && v.y != 0 && v.z != 0 && v.w != 0);
	x /= v.x;
	y /= v.y;
	z /= v.z;
	w /= v.w;
...
FORCEINLINE void VectorDivide(const Vector &a, const Vector &b, Vector &c)
{
	CHECK_VALID(a);
	CHECK_VALID(b);
	DbgAssert((b.x != 0.0f) && (b.y != 0.0f) && (b.z != 0.0f));
	c.x = a.x / b.x;
	c.y = a.y / b.y;
	c.z = a.z / b.z;
}
