...
#if defined(POSIX) && !defined(__ICC)
...
#define FmtStrVSNPrintf(szBuf, nBufSize, refnFormattedLength, bQuietTruncation, pszFormat, lastArg)		\
	...
	{																									\
		...
		(refnFormattedLength) = Q_vsnprintfRet((szBuf), (nBufSize), pszFormat, arg_ptr, &bTruncated);	\
		va_end(arg_ptr);																				\
																										\
		(szBuf)[(nBufSize) - 1] = '\0';																	\
...
public:
	...
	explicit CNumStr(long l)			{ if (sizeof(l) == 4) SetInt32((int32_t)l); else SetInt64((int64_t)l); }
	explicit CNumStr(unsigned long l)	{ if (sizeof(l) == 4) SetUint32((uint32_t)l); else SetUint64((uint64_t)l); }
...
#ifdef _WIN32
	inline const char *SetInt8(int8_t n8)			{ NUMSTR_CHECK_FAST(n8,   uint8_t)	_itoa((int32_t)n8, m_szBuf, 10); m_nLength = strlen(m_szBuf); return m_szBuf; }
	inline const char *SetUint8(uint8_t un8)		{ NUMSTR_CHECK_FAST(un8,  uint8_t)	_itoa((int32_t)un8, m_szBuf, 10); m_nLength = strlen(m_szBuf); return m_szBuf; }
	inline const char *SetInt16(int16_t n16)		{ NUMSTR_CHECK_FAST(n16,  uint16_t)	_itoa((int32_t)n16, m_szBuf, 10); m_nLength = strlen(m_szBuf); return m_szBuf; }
	inline const char *SetUint16(uint16_t un16)		{ NUMSTR_CHECK_FAST(un16, uint16_t)	_itoa((int32_t)un16, m_szBuf, 10); m_nLength = strlen(m_szBuf); return m_szBuf; }
	inline const char *SetInt32(int32_t n32)		{ NUMSTR_CHECK_FAST(n32,  uint32_t)	_itoa(n32, m_szBuf, 10); m_nLength = strlen(m_szBuf); return m_szBuf; }
	inline const char *SetUint32(uint32_t un32)		{ NUMSTR_CHECK_FAST(un32, uint32_t)	_i64toa((int64_t)un32, m_szBuf, 10); m_nLength = strlen(m_szBuf); return m_szBuf; }
	inline const char *SetInt64(int64_t n64)		{ NUMSTR_CHECK_FAST(n64,  uint64_t)	_i64toa(n64, m_szBuf, 10); m_nLength = strlen(m_szBuf); return m_szBuf; }
	inline const char *SetUint64(uint64_t un64)		{ NUMSTR_CHECK_FAST(un64, uint64_t)	_ui64toa(un64, m_szBuf, 10); m_nLength = strlen(m_szBuf); return m_szBuf; }
#else
	inline const char *SetInt8(int8_t n8)			{ NUMSTR_CHECK_FAST(n8,   uint8_t)	m_nLength = Q_snprintf(m_szBuf, sizeof(m_szBuf), "%d", (int32_t)n8); return m_szBuf; }
	inline const char *SetUint8(uint8_t un8)		{ NUMSTR_CHECK_FAST(un8,  uint8_t)	m_nLength = Q_snprintf(m_szBuf, sizeof(m_szBuf), "%d", (int32_t)un8); return m_szBuf; }
	inline const char *SetInt16(int16_t n16)		{ NUMSTR_CHECK_FAST(n16,  uint16_t)	m_nLength = Q_snprintf(m_szBuf, sizeof(m_szBuf), "%d", (int32_t)n16); return m_szBuf; }
	inline const char *SetUint16(uint16_t un16)		{ NUMSTR_CHECK_FAST(un16, uint16_t)	m_nLength = Q_snprintf(m_szBuf, sizeof(m_szBuf), "%d", (int32_t)un16); return m_szBuf; }
	inline const char *SetInt32(int32_t n32)		{ NUMSTR_CHECK_FAST(n32,  uint32_t)	m_nLength = Q_snprintf(m_szBuf, sizeof(m_szBuf), "%d", n32); return m_szBuf; }
	inline const char *SetUint32(uint32_t un32)		{ NUMSTR_CHECK_FAST(un32, uint32_t)	m_nLength = Q_snprintf(m_szBuf, sizeof(m_szBuf), "%u", un32); return m_szBuf; }
	inline const char *SetInt64(int64_t n64)		{ NUMSTR_CHECK_FAST(n64,  uint64_t)	m_nLength = Q_snprintf(m_szBuf, sizeof(m_szBuf), "%lld", n64); return m_szBuf; }
	inline const char *SetUint64(uint64_t un64)		{ NUMSTR_CHECK_FAST(un64, uint64_t)	m_nLength = Q_snprintf(m_szBuf, sizeof(m_szBuf), "%llu", un64); return m_szBuf; }
#endif

	inline const char *SetDouble(double f)			{ if (f == 0.0  && !std::signbit(f)) NUMSTR_FAST_DIGIT(0); if (f == 1.0 ) NUMSTR_FAST_DIGIT(1); m_nLength = _snprintf(m_szBuf, sizeof(m_szBuf), "%.18g", f); return m_szBuf; }
	inline const char *SetFloat(float f)			{ if (f == 0.0f && !std::signbit(f)) NUMSTR_FAST_DIGIT(0); if (f == 1.0f) NUMSTR_FAST_DIGIT(1); m_nLength = _snprintf(m_szBuf, sizeof(m_szBuf), "%.18g", f); return m_szBuf; }

	//SDR_PUBLIC inline const char* SetHexUint64(uint64_t un64)	{ Q_binarytohex((byte *)&un64, sizeof(un64), m_szBuf, sizeof(m_szBuf)); m_nLength = strlen(m_szBuf); return m_szBuf; }
...
#undef NUMSTR_CHECK_FAST
	...
	void AddQuotes()
	{
		Assert(m_nLength + 2 <= ARRAYSIZE(m_szBuf));
...
// CFMTSTR_MAX_STACK_ALLOC is protected so it can be specialized for architecture, bitness, or whatnot.
...
public:
	...
	void AppendIndent(uint32_t unCount, char chIndent = '\t');
...
void CFmtStrN<SIZE_BUF, QT, ON_STACK>::AppendIndent(uint32_t unCount, char chIndent)
{
...
{
	int cubPrinted = Q_vsnprintf(BaseClass::m_szBuf+Length(), SIZE_BUF - Length(), pchFormat, args);
...
// Default-sized string formatter
