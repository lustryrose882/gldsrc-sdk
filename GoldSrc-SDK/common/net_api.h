...
typedef struct net_api_s
{
	...
	void		(*SendRequest)(int context, int request, int flags, double timeout, struct netadr0_s *remote_address, net_api_response_func_t response);
	void		(*CancelRequest)(int context);
	void		(*CancelAllRequests)();
	const char	*(*AdrToString)(struct netadr0_s *a);
	int			(*CompareAdr)(struct netadr0_s *a, struct netadr0_s *b);
	int			(*StringToAdr)(const char *s, struct netadr0_s *a);
	const char *(*ValueForKey)(const char *s, const char *key);
	void		(*RemoveKey)(char *s, const char *key);
	qboolean	(*SetValueForKey)(char *s, const char *key, const char *value, unsigned int maxsize);
} net_api_t;

extern net_api_t netapi;

#endif // #if !defined(NET_APIH)
