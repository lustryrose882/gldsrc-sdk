...
#if defined(_WIN32)
	constexpr auto STDIO_FILESYSTEM_LIB = "filesystem_stdio.dll";
	constexpr auto STEAM_FILESYSTEM_LIB = "filesystem_steam.dll"; // is obsolete
#elif defined(LINUX)
	constexpr auto STDIO_FILESYSTEM_LIB = "filesystem_stdio.so";
	constexpr auto STEAM_FILESYSTEM_LIB = "filesystem_steam.so"; // is obsolete
#elif defined(OSX)
	constexpr auto STDIO_FILESYSTEM_LIB = "filesystem_stdio.dylib";
	constexpr auto STEAM_FILESYSTEM_LIB = "filesystem_steam.dylib"; // is obsolete
#else
...
typedef enum
{
	FILESYSTEM_SEEK_HEAD = 0,					// Seek relative to the start of the file
	FILESYSTEM_SEEK_CURRENT,					// Seek relative to the current position in the file
	FILESYSTEM_SEEK_TAIL,						// Seek relative to the end of the file
} FileSystemSeek_t;
...
public:
	...
	// @param pathID Which path ID this should be assigned to. Can be null,
	// in which case the path will only be used if a null path ID is given to methods taking a path ID.
	...
	// Deletes a file
	//
	// @param pRelativePath The relative path to the file. This path can be relative to any search
	//	path that is part of the given path ID.
	// @param pathID If not null, only search paths defined for the given path ID will be used to search for the file.
	virtual void			RemoveFile(const char *pRelativePath, const char *pathID = 0L) = 0;
	...
	//		Uses the standard path syntax, e.g. models/player/freeman creates the directory models,
	//		creates the directory player in that directory, and creates the directory freeman in that directory.
	//
	...
	virtual void			CreateDirHierarchy(const char *path, const char *pathID = 0L) = 0;
	...
	// Closes a file handle that was previously returned by a call to Open.
	...
	virtual char			*ParseFile(char *pFileBytes, char *pToken, bool *pWasQuoted = nullptr) = 0;

	// Converts a full (absolute) path to a relative path.
	// Note: This function does not take a buffer size parameter.
	// Ensure pRelative is large enough to store at least the entire pFullpath string to prevent buffer overflows.
	//
	...
	// Note: This may be called during shutdown.
	// Do not set a function that is not guaranteed to be in memory for at least as long as the filesystem itself.
	//
...
CreateInterfaceFn FileSystem_Init(const char *basePath = nullptr);
