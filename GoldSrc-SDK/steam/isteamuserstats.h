...
// maximum number of details int32_t's storable for a single leaderboard entry
...
public:
	...
	//				int32_t details[3];		// we know this is how many we've stored previously
	...
	// pScoreDetails points to an array of int32_t's, cScoreDetailsCount is the number of int32_t's in the list
	...
	virtual uint64_t GetTrophySpaceRequiredBeforeInstall() = 0;
	...
	virtual bool SetUserStatsData(const void *pvData, uint32_t cubData) = 0;
	...
	// buffer size.
	virtual bool GetUserStatsData(void *pvData, uint32_t cubData, uint32_t *pcubWritten) = 0;
#endif
};

#else

#define ISteamUserStats ISteamUserStats011
...
// Call result for finding a leaderboard, returned as a result of FindOrCreateLeaderboard() or FindLeaderboard()
...
// Call result indicating scores for a leaderboard have been downloaded and are ready to be retrieved, returned as a result of DownloadLeaderboardEntries()
// use CCallResult<> to map this async result to a member function
...
// Call result indicating scores has been uploaded, returned as a result of UploadLeaderboardScore()
// use CCallResult<> to map this async result to a member function
...
struct NumberOfCurrentPlayers_t
{
	...
	uint8_t m_bSuccess;			// 1 if the call was successful
	int32_t m_cPlayers;			// Number of players currently playing
};
...
// Callback indicating that an achievement icon has been fetched
...
// Callback indicating that global achievement percentages are fetched
...
// Call result indicating UGC has been uploaded, returned as a result of SetLeaderboardUGC()
...
// Callback indicating that PS3 trophies have been installed
struct PS3TrophiesInstalled_t
{
	enum { k_iCallback = k_iSteamUserStatsCallbacks + 12 };
	uint64_t	m_nGameID;				// Game these stats are for
	EResult m_eResult;					// The result of the operation
	uint64_t m_ulRequiredDiskSpace;		// If m_eResult is k_EResultDiskFull, will contain the amount of space needed to install trophies

};

// Callback indicating global stats have been received.
// Returned as a result of RequestGlobalStats()
struct GlobalStatsReceived_t
{
	enum { k_iCallback = k_iSteamUserStatsCallbacks + 12 };
	uint64_t	m_nGameID;				// Game global stats were requested for
