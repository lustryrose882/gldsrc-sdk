...
// Defines the largest allowed file size. Cloud files cannot be written
// in a single chunk over 100MB (and cannot be over 200MB total.)
const uint32_t k_unMaxCloudFileChunkSize = 100 * 1024 * 1024;

// Structure that contains an array of const char * strings and the number of those strings
...
const uint32_t k_cchPublishedDocumentTitleMax = 128 + 1;
const uint32_t k_cchPublishedDocumentDescriptionMax = 8000;
const uint32_t k_cchPublishedDocumentChangeDescriptionMax = 8000;
const uint32_t k_unEnumeratePublishedFilesMaxResults = 50;
const uint32_t k_cchTagListMax = 1024 + 1;
const uint32_t k_cchFilenameMax = 260;
const uint32_t k_cchPublishedFileURLMax = 256;
...
// Functions for accessing, reading and writing files stored remotely
// and cached locally
...
#define ISteamRemoteStorage ISteamRemoteStorage010
...
// Sent when the local file cache is fully synced with the server for an app
...
// Sent when the server is fully synced with the local file cache for an app
...
// Status of up and downloads during a sync session
struct RemoteStorageAppSyncProgress_t
{
	...
	uint32_t m_uBytesTransferredThisChunk;		// Bytes transferred this chunk
...
// Sent after we've determined the list of files that are out of sync with the server.
...
// Sent after a conflict resolution attempt.
...
// The result of a call to FileShare()
...
// The result of a call to PublishFile()
...
// The result of a call to EnumerateUserPublishedFiles()
...
// The result of a call to SubscribePublishedFile()
...
// The result of a call to EnumerateSubscribePublishedFiles()
struct RemoteStorageEnumerateUserSubscribedFilesResult_t
{
	...
	int32_t m_nTotalResultCount;
	PublishedFileId_t m_rgPublishedFileId[ k_unEnumeratePublishedFilesMaxResults ];
	uint32_t m_rgRTimeSubscribed[ k_unEnumeratePublishedFilesMaxResults ];
};
...
// The result of a call to UnsubscribePublishedFile()
...
// The result of a call to UGCDownload()
struct RemoteStorageDownloadUGCResult_t
{
	...
	int32_t m_nSizeInBytes;						// The size of the file that was downloaded, in bytes.
...
// The result of GetPublishedItemVoteDetails
...
// The result of a call to UpdateUserPublishedItemVote()
...
// The result of a call to GetUserPublishedItemVoteDetails()
...
struct RemoteStorageEnumeratePublishedFilesByUserActionResult_t
{
	...
	int32_t m_nTotalResultCount;
	PublishedFileId_t m_rgPublishedFileId[ k_unEnumeratePublishedFilesMaxResults ];
	uint32_t m_rgRTimeUpdated[ k_unEnumeratePublishedFilesMaxResults ];
};


// Called periodically while a PublishWorkshopFile is in progress
