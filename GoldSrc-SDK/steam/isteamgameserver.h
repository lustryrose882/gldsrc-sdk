...
#define MASTERSERVERUPDATERPORT_USEGAMESOCKETSHARE	((uint16_t)-1)
...
#define ISteamGameServer ISteamGameServer011
...
struct GSGameplayStats_t
{
	...
	int32_t	m_nRank;					// Overall rank of the server (0-based)
	uint32_t	m_unTotalConnects;			// Total number of clients who have ever connected to the server
	uint32_t	m_unTotalMinutesPlayed;		// Total number of minutes ever played on the server
};
...
struct GSReputation_t
{
	...
	EResult	m_eResult;					// Result of the call;
	uint32_t	m_unReputationScore;	// The reputation score for the game server
	...
	uint32_t	m_unBannedIP;			// The IP of the banned server
	uint16_t	m_usBannedPort;			// The port of the banned server
	uint64_t	m_ulBannedGameID;		// The game ID the banned server is serving
	uint32_t	m_unBanExpires;			// Time the ban expires, expressed in the Unix epoch (seconds since 1/1/1970)
};
