...
// checks if a local game running via Steam client
S_API bool Steam_IsRunning();
...
S_API void SteamAPI_WriteMiniDump(uint32_t uBuildID, EXCEPTION_POINTERS *pvExceptionInfo);
S_API void SteamAPI_SetMiniDumpComment(const char *pchMsg);

// interface pointers, configured by SteamAPI_Init()
S_API ISteamClient012 *SteamClient();
...
S_API ISteamUser016 *SteamUser();
S_API ISteamFriends013 *SteamFriends();
S_API ISteamUtils005 *SteamUtils();
S_API ISteamMatchmaking009 *SteamMatchmaking();
S_API ISteamUserStats011 *SteamUserStats();
S_API ISteamApps005 *SteamApps();
S_API ISteamNetworking005 *SteamNetworking();
S_API ISteamMatchmakingServers002 *SteamMatchmakingServers();
S_API ISteamRemoteStorage010 *SteamRemoteStorage();
S_API ISteamScreenshots001 *SteamScreenshots();
S_API ISteamHTTP002 *SteamHTTP();
...
// Base for callbacks, used only by CCallback, shouldn't be used directly
class CCallbackBase
{
public:
	...
	uint8_t GetFlags() { return m_nCallbackFlags; }
	bool IsRegistered() const { return (m_nCallbackFlags & k_ECallbackFlagsRegistered) == k_ECallbackFlagsRegistered; }
	void SetFlags(uint8_t flags) { m_nCallbackFlags = flags; }
...
// Templated base for callbacks - internal implementation detail
...
// Maps a steam async call result to a class member function
...
// Maps a steam callback to a class member function
//		template params: T = local class, P = parameter struct
...
// Subclass of CCallback which allows default-construction in
...
#define STEAM_CALLBACK(thisclass, param, var) CCallback<thisclass, param, false> var
#define STEAM_CALLBACK_F(thisclass, func, param, var) CCallback<thisclass, param, false> var; void func(param *pParam)

// same as above, but lets you defer the callback binding by calling Register later
#define STEAM_CALLBACK_MANUAL(thisclass, param, var) CCallbackManual<thisclass, param, false> var
#define STEAM_CALLBACK_MANUAL_F(thisclass, func, param, var) CCallbackManual<thisclass, param, false> var; void func(param *pParam)
...
public:
	...
	ISteamUser016				*SteamUser()				{ return m_pSteamUser; }
	ISteamFriends013			*SteamFriends()				{ return m_pSteamFriends; }
	ISteamUtils005				*SteamUtils()				{ return m_pSteamUtils; }
	ISteamMatchmaking009		*SteamMatchmaking()			{ return m_pSteamMatchmaking; }
	ISteamUserStats011			*SteamUserStats()			{ return m_pSteamUserStats; }
	ISteamApps005				*SteamApps()				{ return m_pSteamApps; }
	ISteamMatchmakingServers002	*SteamMatchmakingServers()	{ return m_pSteamMatchmakingServers; }
	ISteamNetworking005			*SteamNetworking()			{ return m_pSteamNetworking; }
	ISteamRemoteStorage010		*SteamRemoteStorage()		{ return m_pSteamRemoteStorage; }
	ISteamScreenshots001		*SteamScreenshots()			{ return m_pSteamScreenshots; }
...
private:
	...
	ISteamMatchmaking009		*m_pSteamMatchmaking;
	ISteamUserStats011			*m_pSteamUserStats;
	ISteamApps005				*m_pSteamApps;
	ISteamMatchmakingServers002	*m_pSteamMatchmakingServers;
	ISteamNetworking005			*m_pSteamNetworking;
	ISteamRemoteStorage010		*m_pSteamRemoteStorage;
	ISteamScreenshots001		*m_pSteamScreenshots;
...
class ISteamClient012 *SteamAPI_Init_Internal(CSysModule *&pModule, bool bGameServer = false);

#endif // #if defined(STEAM_API_EXPORTS)
...
S_API void SteamAPI_UseBreakpadCrashHandler(char const *pchVersion, char const *pchDate, char const *pchTime, bool bFullMemoryDumps, void *pvContext, PFNPreMinidumpCallback pfnPreMinidumpCallback);
S_API void SteamAPI_SetBreakpadAppID(uint32_t unAppID);
#endif
