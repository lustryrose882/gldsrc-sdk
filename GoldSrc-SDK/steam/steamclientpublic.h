...
// Avatar sizes, used in ISteamFriends
enum EAvatarSize
{
...
// designed as flags to allow filters masks
enum EAppType
{
...
// Status of a given depot version, these are stored in the DB, don't renumber
...
// Marketing message flags that change how a client should handle them
...
// Possible positions to tell the overlay to show notifications in
...
public:
	...
	static const char *Render(uint64_t ulGameID);		// static method to render a uint64_t representation of a Game ID to a string

	// must include checksum_crc.h first to get this functionality
#if defined(CHECKSUM_CRC_H)
	CGameID(uint32_t nAppID, const char *pchModPath)
	{
		m_ulGameID = 0;
		m_gameID.m_nAppID = nAppID;
		m_gameID.m_nType = k_EGameIDTypeGameMod;
