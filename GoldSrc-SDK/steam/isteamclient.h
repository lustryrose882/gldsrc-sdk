...
// The 32-bit version of gcc has the alignment requirement for uint64_t and double set to
...
//	Interface to creating a new steam instance, or to
...
#define ISteamClient ISteamClient012

#endif // #if !defined(STEAM_API_BUILD)

#define STEAMCLIENT_INTERFACE_VERSION   "SteamClient012"
#define STEAMCLIENTEX_INTERFACE_VERSION "SteamClient013"

// Base values for callback identifiers, each callback must
// have a unique ID.
...
constexpr HSteamPipe k_hSteamPipeIndividual = 1;
constexpr HSteamPipe k_hSteamPipeGameServer = 2;

constexpr HSteamUser k_hSteamUser           = 1;
constexpr HSteamUser k_hSteamUserGameServer = 2;
...
public:
	...
	virtual uint32_t GetCallbackID() const = 0;
	virtual uint8_t *GetFixedData() const = 0;
	virtual uint32_t GetFixedSize() const = 0;
	virtual uint32_t GetNumMemberVariables() const = 0;
	virtual bool GetMemberVariable(uint32_t index, uint32_t &varOffset, uint32_t &varSize, uint32_t &varCount, const char **pszName, const char **pszType) const = 0;
};
...
#define CALLBACK_MEMBER(varidx, vartype, varname) \
	...
	static void GetMemberVar_##varidx(unsigned int &varOffset, unsigned int &varSize, uint32_t &varCount, const char **pszName, const char **pszType) { \
...
public: callbackname##_t m_Data; \
	...
	virtual uint32_t  GetCallbackID() const { return callbackname##_t::k_iCallback; } \
	virtual uint32_t  GetFixedSize() const { return sizeof(m_Data); } \
	virtual uint8_t  *GetFixedData() const { return (uint8_t*)&m_Data; } \
	virtual uint32_t  GetNumMemberVariables() const { return numvars; } \
	virtual bool    GetMemberVariable(uint32_t index, uint32_t &varOffset, uint32_t &varSize,  uint32_t &varCount, const char **pszName, const char **pszType) const { \
...
public: callbackname##_t m_Data; \
	...
	virtual uint32_t  GetFixedSize() const { return sizeof(m_Data); } \
	virtual uint8_t  *GetFixedData() const { return (uint8_t*)&m_Data; } \
	virtual uint32_t  GetNumMemberVariables() const { return 0; } \
	virtual bool    GetMemberVariable(uint32_t index, uint32_t &varOffset, uint32_t &varSize,  uint32_t &varCount, const char **pszName, const char **pszType) const { return false; } \
	}; \
