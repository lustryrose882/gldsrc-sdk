...
#define ISteamMatchmaking ISteamMatchmaking009
...
// Callback interface for receiving responses after a server list refresh
...
class _ISteamMatchmakingServerListResponse_v001
{
public:
	// Server has responded ok with updated data
...
// Callback interface for receiving responses after requesting details on
...
// Callback interface for receiving responses after requesting rules
...
// Functions for match making services for clients to get to game lists and details
#if !defined(STEAM_API_BUILD)
...
#define ISteamMatchmakingServers ISteamMatchmakingServers002
...
// game server flags
const uint32_t k_unFavoriteFlagNone				= 0x00;
const uint32_t k_unFavoriteFlagFavorite			= 0x01; // this game favorite entry is for the favorites list
const uint32_t k_unFavoriteFlagHistory			= 0x02; // this game favorite entry is for the history list
const uint32_t k_unFavoriteFlagFavoriteBugged	= 0x03; // same as k_unFavoriteFlagFavorite, but bugged bytes order


// Used in ChatInfo messages - fields specific to a chat member - must fit in a uint32_t
...
struct FavoritesListChanged_t
{
	...
	uint32_t m_nIP;				// an IP of 0 means reload the whole list, any other value means just one server
	uint32_t m_nQueryPort;
	uint32_t m_nConnPort;
	uint32_t m_nAppID;
	uint32_t m_nFlags;
	bool m_bAdd;				// true if this is adding the entry, otherwise it is a remove
	uint32_t m_nAccountID;		// offs: 24
};
...
struct LobbyChatMsg_t
{
	...
	uint64_t m_ulSteamIDLobby;			// the lobby id this is in
	uint64_t m_ulSteamIDUser;			// steamID of the user who has sent this message
	uint8_t m_eChatEntryType;			// type of message
	uint32_t m_iChatID;				// index of the chat entry to lookup
};
...
struct LobbyGameCreated_t
{
	...
	uint64_t m_ulSteamIDLobby;		// the lobby we were in
	uint64_t m_ulSteamIDGameServer;	// the new game server that has been created or found for the lobby members
	uint32_t m_unIP;					// IP & Port of the game server (if any)
	uint16_t m_usPort;
};
...
struct LobbyMatchList_t
{
	...
	uint32_t m_nLobbiesMatching;		// Number of lobbies that matched search criteria and we have SteamIDs for
};
...
struct LobbyKicked_t
{
	...
	uint64_t m_ulSteamIDLobby;			// Lobby
	uint64_t m_ulSteamIDAdmin;			// User who kicked you - possibly the ID of the lobby itself
	uint8_t m_bKickedDueToDisconnect;		// true if you were kicked from the lobby due to the user losing connection to Steam (currently always true)
};
...
// Result of CheckForPSNGameBootInvite
// m_eResult == k_EResultOK on success
